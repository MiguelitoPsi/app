...
export const GameProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const tasks = useMemo(() => {
    // Regular tasks
    const regularTasks = tasksData.map((task: any) => ({
      id: task.id,
      title: task.title,
    ...
    const fromTherapist = therapistTasksData.map((task: any) => ({
      id: task.id,
      title: task.title,
      priority: (task.priority || 'medium') as 'high' | 'medium' | 'low',
  ...
  const addTask = (taskData: Omit<Task, 'completed' | 'id'>) => {
    ...
    try {
      ...
      utils.client.task.create
        .mutate({
          ...
          weekDays: taskData.weekDays || undefined,
          monthDays: taskData.monthDays || undefined,
          startTime: (taskData.startTime as string) || undefined,
          endTime: (taskData.endTime as string) || undefined,
          metadata: (taskData.metadata as any) || undefined,
        })
        .then(() => {
