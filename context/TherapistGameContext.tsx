...
const TherapistGameContext = createContext<TherapistGameContextType | undefined>(undefined)

/* ============================================
 * PROVIDER
 * ============================================ */

export const TherapistGameProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const { data: userProfile, isLoading: isLoadingProfile } = trpc.user.getProfile.useQuery(
    undefined,
    {
  ...
  useEffect(() => {
    if (userProfile?.preferences) {
      const savedTheme = (userProfile.preferences as Record<string, unknown>)?.theme as
        | 'light'
        | 'dark'
        | undefined
      if (savedTheme) {
  ...
  return <TherapistGameContext.Provider value={value}>{children}</TherapistGameContext.Provider>
}
...
export function useTherapistGame(): TherapistGameContextType {
  ...
  if (context === undefined) {
    throw new Error('useTherapistGame must be used within a TherapistGameProvider')
  }
  return context
}
