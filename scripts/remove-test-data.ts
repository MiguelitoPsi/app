...
async function main() {
  try {
    ...
    if (testUser) {
      ...
      const profileDeleted = await db.delete(therapistProfiles).where(eq(therapistProfiles.therapistId, testUser.id))
    ...
    const [extraProfile] = await db.select().from(therapistProfiles).where(eq(therapistProfiles.username, '00000000000'))
    if (extraProfile) {
      console.log(`Found extra profile with username 00000000000 (ID: ${extraProfile.therapistId}). Deleting...`)
