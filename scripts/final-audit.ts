...
async function main() {
  ...
  const [profile] = await db.select().from(therapistProfiles).where(eq(therapistProfiles.therapistId, userId))
  console.log('Therapist Profile:', profile ? 'OK' : 'MISSING')
  
  const [stats] = await db.select().from(therapistStats).where(eq(therapistStats.therapistId, userId))
  console.log('Therapist Stats:', stats ? 'OK' : 'MISSING')
  
  const financial = await db.select().from(therapistFinancial).where(eq(therapistFinancial.therapistId, userId))
  console.log('Financial Records:', financial.length)
  
  const patientsCount = await db.select().from(psychologistPatients).where(eq(psychologistPatients.psychologistId, userId))
  console.log('Linked Patients:', patientsCount.length)
  
  const invites = await db.select().from(patientInvites).where(eq(patientInvites.psychologistId, userId))
