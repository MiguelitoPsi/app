...
function SignInForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const accountStatusResponse = await fetch('/api/auth/check-account-status', {
        method: 'GET',
        credentials: 'include',
      })

      if (!accountStatusResponse.ok) {
        throw new Error('Não foi possível validar o acesso da conta.')
      }
      ...
      if (accountStatus.isBlocked) {
        ...
        throw new Error(accountStatus.message || 'Sua conta está bloqueada para acesso.')
      }
