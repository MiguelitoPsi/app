...
export default function SubscriptionPage() {
  ...
  const { data: paymentHistoryData } =
    trpc.therapistSubscription.getPaymentHistory.useQuery({
      limit: 10,
      offset: 0,
    });
  ...
  const { data: currentPaymentLink } =
    trpc.therapistSubscription.getCurrentPaymentLink.useQuery(undefined, {
  ...
  const cancelMutation = trpc.therapistSubscription.cancel.useMutation({
    onSuccess: () => {
      toast.success(
        "Assinatura cancelada. Você ainda tem acesso até o final do período.",
      );
    },
    onError: (err) => toast.error(err.message),
  });
                ...
                >
                  ...
                  <div className="mb-6 space-y-2">
                    {plan.maxPatients !== null && plan.maxPatients > 0 && (
                      <FeatureItem
                        enabled
                        label={`Até ${plan.maxPatients} pacientes`}
                      />
                    )}
                    {plan.maxPatients === 0 && (
                      <FeatureItem enabled label="Pacientes ilimitados" />
                    )}
                    {plan.features &&
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                {paymentHistory.map((pmt) => (
                  <tr key={pmt.id} className="bg-white dark:bg-slate-800/50">
    ...
    >
      ...
      {isPastDue && subscriptionStatus.daysRemaining !== undefined && (
        <div className="mt-3 rounded-lg bg-amber-500/10 px-4 py-2 text-sm text-amber-600 dark:text-amber-400">
          ...
          Pagamento pendente. Você tem {subscriptionStatus.daysRemaining} dias
          restantes de acesso.
        </div>
      )}
    </div>
