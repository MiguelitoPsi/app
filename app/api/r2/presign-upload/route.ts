...
import { generatePresignedUploadUrl, generateR2Key, getPublicUrl, R2_CONFIG } from '@/lib/r2/client'
...
export async function POST(request: Request) {
  try {
    ...
    const hasRequiredFields = patientId && filename && contentType && fileSize && purpose
    if (!hasRequiredFields) {
      return NextResponse.json(
        {
          error: 'Campos obrigatórios: patientId, filename, contentType, fileSize, purpose',
        },
        { status: 400 }
      )
    }
