...
interface AnalyzePatientDataInput {
...
interface AnalyzePatientDataOutput {
...
export async function POST(request: Request) {
  try {
    ...
    const totalDataPoints =
      data.journalEntries.length + data.transcriptions.length + data.moodHistory.length
    ...
    const prompt = buildAnalyzePatientDataPrompt({
      ...
      birthDateLine: data.patientInfo.birthDate
        ...
        : undefined,
      genderLine: data.patientInfo.gender ? `Gênero: ${data.patientInfo.gender}` : undefined,
          ...
          totalDataPoints < 5
            ? [
                ...
                data.journalEntries.length < 3
                  ...
                  : '',
                data.transcriptions.length < 2 ? 'Adicione mais transcrições de sessões' : '',
              ].filter(Boolean)
            : [],
      },
    }
    ...
    return NextResponse.json({ error: `Erro ao analisar dados: ${errorMessage}` }, { status: 500 })
  }
}
