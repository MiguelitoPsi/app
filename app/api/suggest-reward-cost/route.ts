  ...
  try {
    ...
    if (!title) {
      return NextResponse.json(
        { error: "título da recompensa é obrigatório" },
        { status: 400 },
      );
    }
    ...
    if (!apiKey || apiKey === "PLACEHOLDER_API_KEY" || apiKey.trim() === "") {
      console.warn(
        "GEMINI_API_KEY not configured or is a placeholder, using fallback values",
      );
      return NextResponse.json({
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return NextResponse.json({
          suggestedCost: Math.min(
            500,
            Math.max(10, Number(parsed.suggestedCost) || 50),
          ),
          reasoning:
            parsed.reasoning ||
            "Sugestão baseada na categoria e valor da recompensa.",
