...
import { asaasWebhookEvents, subscriptionPayments, therapistSubscriptions } from '@/lib/db/schema'
...
async function handlePaymentEvent(payload: AsaasWebhookPayload) {
  ...
  if (!asaasSubscriptionId) {
    console.log('[Asaas Webhook] Payment is not subscription-related, skipping')
    return
  }
  ...
  if (!sub) {
    console.warn('[Asaas Webhook] No matching subscription for:', asaasSubscriptionId)
    return
  }
...
async function handleSubscriptionEvent(payload: AsaasWebhookPayload) {
  ...
  if (!subscription) {
    console.warn('[Asaas Webhook] Subscription event without subscription data')
    return
  }
