...
function getMoodFromScore(score: number) {
  return (
    MOOD_MAP.find((m) => score >= m.range[0] && score <= m.range[1]) ??
    MOOD_MAP[3]
  );
}
...
const STATS_CARDS = [
  {
    label: "Diários",
    value: MOCK_STATS.journalEntries,
    icon: BookOpen,
  ...
  {
    label: "Meditação",
    value: `${MOCK_STATS.meditationMinutes}min`,
    icon: Brain,
  ...
  {
    label: "Sequência",
    value: `${MOCK_STATS.currentStreak} dias`,
    icon: Sparkles,
...
export default function HomeMain() {
  ...
  const firstName = stats.name ? stats.name.split(" ")[0] : "";
        ...
        >
          <div className="p-4" style={{ backgroundColor: moodColor }}>
            ...
            <p className="mt-0.5 text-[10px] text-white/70">
              Seu desempenho dos últimos 7 dias
            </p>
          </div>
          <div
          ...
          >
            <div className="flex flex-col items-center py-4">
              ...
              <span className="text-[10px] font-medium text-[#8a7e6b]">
                Humor Médio
              </span>
            </div>
            <div className="flex flex-col items-center py-4">
              <span className="text-lg font-extrabold text-[#5a5243]">
                {MOCK_STATS.sleepHours}
              </span>
              <span className="text-[10px] font-medium text-[#8a7e6b]">
                Sono Médio
              </span>
