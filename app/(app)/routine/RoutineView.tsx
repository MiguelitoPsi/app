...
export const RoutineView: React.FC = () => {
  const { tasks, toggleTask, deleteTask, urgentOverdueTasks, dismissUrgentTask } = useGame()
  ...
  const [_alertTitle, setAlertTitle] = useState('Atenção')

  // Overdue Alert Modal State
  const [_showOverdueAlert, setShowOverdueAlert] = useState(false)
  ...
  const getPriorityColor = (p: string) => PRIORITY_COLORS[p] || 'rgb(148, 163, 184)'
                ...
                >
                  <span
                    className={`text-xs font-medium ${
                      isSelected ? 'text-[#a1c797]' : 'text-slate-500 dark:text-slate-500'
                    }`}
                  >
        ...
        <main className='flex-1 px-6 pb-28' id='main-content'>
          {displayTasks.length === 0 ? (
            <div className='mt-8 flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-6 text-center dark:border-slate-800 dark:bg-slate-900/50'>
              ...
              <h4 className='mb-1 text-sm font-bold text-[#a1c797]'>Nenhuma tarefa encontrada</h4>
            ...
            <div className='relative pl-2'>
              {displayTasks.map((task, index) => {
                ...
                return (
                  <div className='flex items-stretch gap-4' key={task.id}>
                    {/* Timeline column */}
                    ...
                    {/* Task content */}
                    <div className={`min-w-0 flex-1 pb-6 ${task.completed ? 'opacity-50' : ''}`}>
