...
export default function NewTaskPage() {
  ...
  }, [taskDate, frequency, selectedWeekDays]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (firstTimestamp) {
      ...
      if (priority === "high") {
        const highCount = tasksForDate.filter(
          (t) => t.priority === "high",
        ).length;
        if (highCount >= 2) {
      ...
      if (priority === "medium") {
        const mediumCount = tasksForDate.filter(
          (t) => t.priority === "medium",
        ).length;
        if (mediumCount >= 5) {
          const dateStr = targetDate.toLocaleDateString("pt-BR");
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        animate={isExiting ? { y: "100%" } : { y: 0 }}
        transition={
          isExiting ? { duration: 0.5, ease: [0.4, 0, 1, 1] } : ENTER_TRANSITION
        }
        onAnimationComplete={() => {
                  ...
                  {["D", "S", "T", "Q", "Q", "S", "S"].map((day, index) => {
                    ...
                    return (
                      <button
                        ...
                        style={
                          isSelected ? { backgroundColor: PRIMARY } : undefined
                        }
                        key={index}
                        onClick={() => {
                          if (isSelected) {
