 ...
 */
export function TherapistLevelUpModal({ newLevel, rank, onClose }: TherapistLevelUpModalProps) {
      ...
      >
        <div className='relative overflow-hidden rounded-2xl border border-emerald-500/30 bg-slate-900 p-8 text-center shadow-2xl shadow-emerald-500/20'>
          ...
          <p className='mb-4 text-slate-300' id='therapist-levelup-description'>
            ...
            <span className='text-xl font-bold text-emerald-400'>{newLevel}</span>
          </p>
          ...
          <div className='mb-6 rounded-xl bg-gradient-to-br from-emerald-600/30 to-teal-600/30 p-4'>
            <div className='mb-2 flex items-center justify-center gap-2'>
              ...
              <h3 className='text-xl font-bold text-emerald-300'>{rank.name}</h3>
            </div>
                ...
                <ul className='space-y-1'>
                  {rank.benefits.map((benefit, index) => (
                    <li
                      ...
                      key={index}
                    >
                      <Award aria-hidden='true' className='h-4 w-4 text-emerald-400' />
                      {benefit}
                    </li>
                  ))}
                </ul>
          ...
          <div aria-hidden='true' className='mb-8 flex items-center justify-center gap-4'>
            <div className='text-sm font-medium text-slate-500'>Nível {newLevel - 1}</div>
            <div className='h-1 w-12 overflow-hidden rounded-full bg-slate-700'>
              <div className='animate-shine h-full w-full bg-emerald-500' />
            </div>
            <div className='text-lg font-bold text-emerald-400'>Nível {newLevel}</div>
          </div>
