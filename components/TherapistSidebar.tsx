...
export const TherapistSidebar: React.FC = memo(function TherapistSidebarComponent() {
  ...
  return (
    <aside className='fixed left-0 top-0 z-40 hidden h-screen w-48 border-r border-slate-200 bg-white/95 backdrop-blur transition-colors duration-300 lg:block dark:border-slate-700 dark:bg-slate-900/95'>
      <div className='flex h-full flex-col'>
        {/* Logo */}
        <div className='flex h-12 items-center gap-2 border-b border-slate-200 px-3 dark:border-slate-700'>
          <div className='flex h-7 w-7 items-center justify-center rounded-md bg-violet-600'>
            <span className='text-sm'>🧠</span>
          </div>
          <span className='text-sm font-bold text-slate-800 dark:text-white'>Terapeuta</span>
        </div>
            ...
            <div className='space-y-2'>
              <div className='flex items-center justify-between'>
                <div>
                  ...
                  <p className='text-xs font-bold text-slate-800 dark:text-white'>Nível {stats.level}</p>
                </div>
                {stats.currentStreak >= 3 && (
                  <span
                    className='flex items-center gap-0.5 rounded bg-orange-500/20 px-1.5 py-0.5 text-[10px] font-medium text-orange-600 dark:text-orange-400'
        ...
        <div className='border-t border-slate-200 p-2 dark:border-slate-700'>
          <button
            className='flex w-full items-center gap-2 rounded px-2 py-1.5 text-xs font-medium text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-white'
