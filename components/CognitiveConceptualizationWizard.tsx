...
interface AssumptionSuggestion {
  id: string
  assumption: string
...
interface ChildhoodDataSuggestion {
  id: string
  content: string
...
interface CognitiveConceptualizationWizardProps {
  isOpen: boolean
  onClose: () => void
  patientId: string
  patientName: string
...
export function CognitiveConceptualizationWizard({
  ...
  const [loadingMessage, setLoadingMessage] = useState('Buscando dados do paciente...')
  const [aiSuggestions, setAiSuggestions] = useState<AISuggestions | null>(null)
  const [formData, setFormData] = useState<CognitiveFormData>(initialFormData)
  const [isSaving, setIsSaving] = useState(false)

  // Selected items state
  const [selectedChildhood, setSelectedChildhood] = useState<Set<string>>(new Set())
  ...
  const [selectedSituations, setSelectedSituations] = useState<(string | null)[]>([
    null,
    null,
    null,
  ])
  ...
  const { data: moodHistory } = trpc.therapistReports.getPatientMoodHistory.useQuery(
    { patientId, limit: 50 },
    { enabled: isOpen && !!patientId }
  )

  const { data: documents } = trpc.therapistReports.getPatientDocuments.useQuery(
    { patientId, limit: 20 },
    { enabled: isOpen && !!patientId }
  )
  ...
  const saveMutation = trpc.therapistReports.saveCognitiveConceptualization.useMutation({
    onSuccess: () => {
        ...
        try {
          ...
          setAiSuggestions({
            ...
            dataQuality: {
              ...
              suggestions: ['Ocorreu um erro na análise. Você pode preencher manualmente.'],
            },
          })
        }
      }
  ...
  useEffect(() => {
    ...
    // Update core belief
    if (selectedBelief) {
      const belief = aiSuggestions.beliefSuggestions.find((b) => b.id === selectedBelief)
      if (belief) {
        setFormData((prev) => ({
          ...prev,
    ...
    selectedSituations.forEach((sitId, index) => {
      if (sitId) {
        const situation = aiSuggestions.situationSuggestions.find((s) => s.id === sitId)
        if (situation) {
          const key = `situation${index + 1}` as 'situation1' | 'situation2' | 'situation3'
          setFormData((prev) => ({
            ...prev,
            [key]: {
  ...
  const handleSave = () => {
    ...
    saveMutation.mutate({
      ...
      situations: {
        situation1: formData.situation1.situation ? formData.situation1 : undefined,
        situation2: formData.situation2.situation ? formData.situation2 : undefined,
        situation3: formData.situation3.situation ? formData.situation3 : undefined,
      },
  ...
  const updateSituationField = (index: 0 | 1 | 2, field: keyof SituationData, value: string) => {
    const key = `situation${index + 1}` as 'situation1' | 'situation2' | 'situation3'
    setFormData((prev) => ({
      ...prev,
      [key]: {
        ...
        <div className='flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-700'>
          <div className='flex items-center gap-3'>
            <div className='flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-purple-400 to-indigo-600'>
              <RiBrain2Line className='h-6 w-6 text-white' />
            </div>
            <div>
        ...
        <div className='flex-1 overflow-y-auto p-6'>
          ...
          {step === 0 && (
            <div className='flex flex-col items-center justify-center py-16'>
              <div className='relative mb-6'>
                ...
                <RiSparklingLine className='absolute inset-0 m-auto h-8 w-8 text-purple-500' />
              </div>
          ...
          {step === 1 && (
            <div className='space-y-6'>
              ...
              {aiSuggestions && aiSuggestions.childhoodSuggestions.length > 0 ? (
                ...
                <div className='bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4'>
                  <p className='text-sm text-amber-700 dark:text-amber-400'>
                    Não foram encontrados dados de infância nos registros. Você pode adicionar
                    manualmente abaixo.
                  </p>
                </div>
              )}
              ...
              <div className='pt-4'>
                <label className='block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2'>
                  <RiPencilLine className='inline h-4 w-4 mr-1' />
          ...
          {step === 2 && (
            <div className='space-y-6'>
              <div className='flex items-center gap-2 mb-4'>
                <RiBrain2Line className='h-5 w-5 text-purple-500' />
                <h3 className='text-lg font-semibold text-slate-800 dark:text-white'>
                  Crença Central
                </h3>
              </div>
              ...
              {aiSuggestions && aiSuggestions.beliefSuggestions.length > 0 ? (
                <div className='space-y-3'>
                  <p className='text-sm text-slate-600 dark:text-slate-400'>
                    Selecione a crença central mais adequada ou escreva uma personalizada:
                  </p>
                  ...
                  >
                    <div className='flex items-center gap-3'>
                      ...
                      <span className='text-slate-600 dark:text-slate-300'>
                        <RiAddLine className='inline h-4 w-4 mr-1' />
                ...
                <div className='bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4'>
                  <p className='text-sm text-amber-700 dark:text-amber-400'>
                    Não foi possível identificar crenças centrais nos dados. Por favor, adicione
                    manualmente.
                  </p>
                </div>
              )}
              ...
              <div className='pt-4'>
                <label className='block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2'>
                  <RiPencilLine className='inline h-4 w-4 mr-1' />
                  {selectedBelief === 'custom' ? 'Escreva a crença central:' : 'Refinar crença:'}
                </label>
                <textarea
                  className='w-full rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-4 py-3 text-slate-700 dark:text-slate-200 focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-500/20'
                  onChange={(e) =>
          ...
          {step === 3 && (
            <div className='space-y-6'>
              <div className='flex items-center gap-2 mb-4'>
                <RiScalesLine className='h-5 w-5 text-amber-500' />
                <h3 className='text-lg font-semibold text-slate-800 dark:text-white'>
                  Suposições Condicionais / Regras
                </h3>
              </div>

              {aiSuggestions && aiSuggestions.assumptionSuggestions.length > 0 ? (
                <div className='space-y-3'>
                  <p className='text-sm text-slate-600 dark:text-slate-400'>
                ...
                <div className='bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4'>
                  <p className='text-sm text-amber-700 dark:text-amber-400'>
                    Não foram identificadas suposições condicionais. Adicione manualmente.
                  </p>
                </div>
              )}
          ...
          {step === 4 && (
            <div className='space-y-6'>
              <div className='flex items-center gap-2 mb-4'>
                <RiPulseLine className='h-5 w-5 text-sky-500' />
                <h3 className='text-lg font-semibold text-slate-800 dark:text-white'>
                  Estratégias Compensatórias
                </h3>
              </div>
              ...
              {aiSuggestions && aiSuggestions.strategySuggestions.length > 0 ? (
                ...
                <div className='bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4'>
                  <p className='text-sm text-amber-700 dark:text-amber-400'>
                    Não foram identificadas estratégias compensatórias. Adicione manualmente.
                  </p>
                </div>
              )}
          ...
          {step === 5 && (
            <div className='space-y-6'>
              <div className='flex items-center gap-2 mb-4'>
                <RiFileTextLine className='h-5 w-5 text-green-500' />
                <h3 className='text-lg font-semibold text-slate-800 dark:text-white'>
                  Situações (até 3)
                </h3>
              </div>
              ...
              <p className='text-sm text-slate-600 dark:text-slate-400'>
                Adicione situações específicas com pensamentos automáticos, emoções e
                comportamentos. Você pode selecionar registros existentes ou preencher manualmente.
              </p>
              ...
              {[0, 1, 2].map((index) => {
                ...
                const hasSuggestions =
                  ...
                  aiSuggestions.situationSuggestions.filter(
                    (s) => !selectedSituations.includes(s.id) || selectedSituations[index] === s.id
                  ).length > 0
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      {isExpanded ? (
                        <RiArrowUpSLine className='h-5 w-5 text-slate-400' />
                      ) : (
                        <RiArrowDownSLine className='h-5 w-5 text-slate-400' />
                      )}
                    </button>
                      ...
                      <div className='px-4 pb-4 space-y-4'>
                                ...
                                .map((suggestion) => (
                                  <button
                                    className={`text-left p-3 rounded-lg border transition-all max-w-xs ${
                                      selectedSituations[index] === suggestion.id
                                  ...
                                  >
                                    ...
                                    <p className='text-xs text-slate-500 mt-1'>
                                      {suggestion.sourceDate} • {suggestion.emotion}
                                    </p>
                                  </button>
                                ))}
                            </div>
                        ...
                        <div className='border-t border-slate-200 dark:border-slate-700 pt-4'>
                          <p className='text-sm font-medium text-slate-600 dark:text-slate-400 mb-3'>
                            {selectedSituations[index] ? 'Editar dados:' : 'Preencher manualmente:'}
                          </p>
            ...
            <div className='space-y-6'>
              ...
              <div className='bg-slate-50 dark:bg-slate-900 rounded-xl p-6 space-y-6'>
                ...
                <div className='flex items-center justify-between border-b border-slate-200 dark:border-slate-700 pb-4'>
                  ...
                  <RiBrain2Line className='h-8 w-8 text-purple-500' />
                </div>
                    ...
                    <table className='w-full text-sm'>
                      <thead>
                        <tr className='border-b border-slate-300 dark:border-slate-700'>
                          ...
                          <th className='text-left py-2 text-slate-500 font-medium'>Situação</th>
                          <th className='text-left py-2 text-slate-500 font-medium'>P.A.</th>
                          <th className='text-left py-2 text-slate-500 font-medium'>Emoção</th>
                      ...
                      <tbody>
                        {[formData.situation1, formData.situation2, formData.situation3].map(
                          (sit, idx) =>
                            sit.situation && (
                              <tr
