...
export function UnlinkedPatientModal() {
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      if (
        event.data?.type === "USER_SUSPENDED" ||
        event.data?.type === "PATIENT_UNLINKED"
      ) {
        forceCheck();
      }
    };
  ...
  const encouragingMessage =
    DISCHARGE_MESSAGES[
      Math.abs(suspensionData.unlinkedByTherapistName?.length ?? 0) %
        DISCHARGE_MESSAGES.length
    ];
        ...
        <div className="mx-4 flex h-[90vh] w-full max-w-2xl flex-col rounded-xl border border-violet-500/50 bg-slate-900 shadow-2xl">
          ...
          <div className="space-y-3 border-b border-slate-700 p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {(["all", "online", "presential", "both"] as const).map(
                (type) => (
                  <button
          ...
          <div className="border-t border-slate-700 p-4">
            <p className="text-center text-slate-400 text-sm">
              Após entrar em contato, o terapeuta enviará um link de convite
              para vincular você ao perfil dele.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm">
        <div className="mx-4 w-full max-w-md rounded-xl border border-emerald-500/50 bg-gradient-to-b from-slate-900 to-emerald-950/30 p-6 shadow-2xl">
          ...
          <div className="mb-6 rounded-lg bg-emerald-900/30 border border-emerald-500/30 p-4">
            <p className="text-center text-emerald-100 italic">
              "{encouragingMessage}"
            </p>
          </div>
          ...
          <div className="mb-4 rounded-lg bg-slate-800/50 p-4">
            <p className="text-slate-300 text-sm">
              Para continuar usando o Nepsis, você pode:
            </p>
            <ul className="mt-2 list-inside list-disc text-slate-400 text-sm">
              <li>
                Buscar um novo terapeuta se desejar continuar o acompanhamento
              </li>
        ...
        <div className="mb-4 rounded-lg bg-red-900/20 border border-red-500/30 p-4">
          ...
          <p className="text-slate-300 text-sm">
            {suspensionData.banReason ||
              "O terapeuta optou por encerrar o acompanhamento."}
          </p>
        </div>

        <div className="mb-6 rounded-lg bg-slate-800/50 p-4">
          <p className="text-slate-300 text-sm">
            Sua conta está temporariamente suspensa. Para continuar usando o
            app, você pode:
          </p>
          <ul className="mt-2 list-inside list-disc text-slate-400 text-sm">
