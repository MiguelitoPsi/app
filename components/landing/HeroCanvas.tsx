...
function SceneContent() {
  ...
  return (
    <>
      <AgentModel
        modelPath="/3d/miguel.glb"
        position={config.psychPos}
        scale={config.psychScale}
      />
      {agents.map((agent) => (
        <group key={agent.id}>
          <AgentModel
          ...
          />
          <DataFlowLine start={agent.position} end={config.psychPos} />
          <ThoughtBubble
            position={[
              agent.position[0],
              agent.position[1] + config.bubbleY,
              agent.position[2],
            ]}
