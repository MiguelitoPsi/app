...
export const journalEntries = sqliteTable("journal_entries", {
  ...
  feedbackViewed: integer("feedback_viewed", { mode: "boolean" }).default(
    false,
  ),
...
export const therapistFinancial = sqliteTable("therapist_financial", {
  ...
  therapistId: text("therapist_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
  accountType: text("account_type", { enum: ["pj", "cpf"] })
    .notNull()
    .default("cpf"),
...
export const therapistGoals = sqliteTable("therapist_goals", {
  ...
  status: text("status", {
    ...
    .notNull()
    .default("active"),
  aiSuggested: integer("ai_suggested", { mode: "boolean" })
    .notNull()
    .default(false),
  completedAt: integer("completed_at", { mode: "timestamp" }),
  createdAt: integer("created_at", { mode: "timestamp" })
...
export const patientTasksFromTherapist = sqliteTable(
  "patient_tasks_from_therapist",
  {
    id: text("id").primaryKey(),
    therapistId: text("therapist_id")
      .notNull()
    ...
    feedbackAt: integer("feedback_at", { mode: "timestamp" }),
    xpReward: integer("xp_reward").notNull().default(20),
    isAiSuggested: integer("is_ai_suggested", { mode: "boolean" })
      .notNull()
      .default(false),
    metadata: text("metadata", { mode: "json" }).$type<{
      icon?: string;
...
// Upload jobs for R2 storage (transcriptions and documents)
export const uploadJobs = sqliteTable("upload_jobs", {
  ...
  purpose: text("purpose", {
    enum: ["transcription", "document"],
  }).notNull(),
  status: text("status", {
    enum: ["pending", "uploading", "completed", "processing", "failed"],
  })
    .notNull()
    .default("pending"),
  metadata: text("metadata", { mode: "json" }).$type<Record<string, unknown>>(),
...
// Session documents (PDFs and images uploaded by therapists) - now uses R2 storage
export const sessionDocuments = sqliteTable("session_documents", {
  ...
  mimeType: text("mime_type").notNull(),
  fileSize: integer("file_size").notNull(),
  // R2 storage fields (replaces fileData)
  r2Key: text("r2_key"), // Key in R2 bucket
  r2Url: text("r2_url"), // Public URL for the file
  // Legacy field - kept for backwards compatibility during migration
  fileData: text("file_data"), // Base64 encoded file data (deprecated)
  description: text("description"),
...
export const cognitiveConceptualization = sqliteTable(
  "cognitive_conceptualization",
  {
    id: text("id").primaryKey(),
    therapistId: text("therapist_id")
      .notNull()
...
// Session transcriptions (synced from external API)
...
export const psychologistSubscriptions = sqliteTable(
...
export type NewTherapistFinancialRecord =
  typeof therapistFinancial.$inferInsert;
...
export type PatientTaskFromTherapist =
  typeof patientTasksFromTherapist.$inferSelect;
export type NewPatientTaskFromTherapist =
  typeof patientTasksFromTherapist.$inferInsert;
...
export type CognitiveConceptualization =
  typeof cognitiveConceptualization.$inferSelect;
export type NewCognitiveConceptualization =
  typeof cognitiveConceptualization.$inferInsert;
export type PsychologistSubscription =
  typeof psychologistSubscriptions.$inferSelect;
export type NewPsychologistSubscription =
  typeof psychologistSubscriptions.$inferInsert;
