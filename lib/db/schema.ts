...
export const therapistGoals = sqliteTable('therapist_goals', {
  ...
  status: text('status', { enum: ['active', 'completed', 'paused', 'cancelled'] })
    .notNull()
    .default('active'),
...
export const weeklyReports = sqliteTable('weekly_reports', {
  ...
  interventionSuggestions: text('intervention_suggestions', { mode: 'json' }).$type<
    {
      suggestion: string
      rationale: string
...
export const patientTasksFromTherapist = sqliteTable('patient_tasks_from_therapist', {
  ...
  frequency: text('frequency', { enum: ['once', 'daily', 'weekly', 'monthly'] })
    ...
    .default('once'),
  weekDays: text('week_days', { mode: 'json' }).$type<number[]>(),
  status: text('status', { enum: ['pending', 'accepted', 'completed', 'rejected'] })
    .notNull()
    .default('pending'),
  completedAt: integer('completed_at', { mode: 'timestamp' }),
...
export const therapySessions = sqliteTable('therapy_sessions', {
  ...
  status: text('status', { enum: ['scheduled', 'completed', 'cancelled', 'no_show'] })
    .notNull()
    .default('scheduled'),
  sessionType: text('session_type', { enum: ['individual', 'couple', 'family', 'group'] })
    .notNull()
    .default('individual'),
  notes: text('notes'),
