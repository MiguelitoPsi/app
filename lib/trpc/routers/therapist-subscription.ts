...
import { router, psychologistProcedure, adminProcedure } from "../trpc";
import {
...
export const therapistSubscriptionRouter = router({
  ...
  getAllSubscriptions: adminProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input?.status) {
        conditions.push(
          sql`${therapistSubscriptions.status} = ${input.status}`,
        );
      }
      if (input?.planId) {
        conditions.push(
          sql`${therapistSubscriptions.planId} = ${input.planId}`,
        );
      }
      ...
      const results = await ctx.db
        ...
        .where(
          and(
            input?.status
              ? eq(therapistSubscriptions.status, input.status)
              : undefined,
            input?.planId
              ? eq(therapistSubscriptions.planId, input.planId)
              : undefined,
            input?.search
