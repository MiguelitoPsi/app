...
export const therapistReportsRouter = router({
  ...
  generateWeeklyReport: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      }% de conclusão). ${journals.length} entradas de diário foram registradas.`
  ...
  // Upload de documento de sessão usando R2 (nova versão)
  uploadSessionDocument: protectedProcedure
    .input(
      z.object({
        ...
        // R2 storage fields
        r2Key: z.string(),
        r2Url: z.string().nullable().optional(),
        // Legacy field - deprecated, use R2 instead
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Validar tamanho do arquivo (máximo 100MB para R2)
      ...
      const document = await db
        ...
        .values({
          ...
          r2Key: input.r2Key,
          r2Url: input.r2Url ?? null,
          fileData: input.fileData ?? null, // Legacy support
          description: input.description,
    ...
    .query(async ({ ctx, input }) => {
      ...
      const documents = await db
        .select({
          ...
          r2Key: sessionDocuments.r2Key,
          r2Url: sessionDocuments.r2Url,
        ...
        .where(
          and(
            ...
            isNull(sessionDocuments.deletedAt)
          )
        )
