...
import { getHomeRouteForRole, isPathAllowedForRole, type UserRole } from '@/lib/auth/roles'
...
const STATIC_FILE_REGEX =
  /\.(png|jpg|jpeg|gif|svg|ico|webp|mp4|webm|mp3|wav|pdf|txt|css|js|json|xml|woff|woff2|ttf|eot|glb|gltf|bin|draco)$/i
...
export function middleware(request: NextRequest) {
  ...
  // Usa o helper oficial do better-auth para detectar a sessão
  ...
  if (isAuthenticated && (path.startsWith('/auth/') || path === '/')) {
    ...
    console.log('[Middleware] Redirecting from', path, 'to:', homeRoute)
