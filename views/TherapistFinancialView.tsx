...
import {
  RiArrowDownCircleLine as ArrowDownCircle,
  RiArrowUpCircleLine as ArrowUpCircle,
  ...
  RiArrowDownLine as TrendingDown,
  RiArrowUpLine as TrendingUp,
  RiUserLine as User,
      ...
      {tabs.map((tab) => (
        ...
        >
          ...
          <span className="sm:hidden">
            {tab.id === "all" ? "Geral" : tab.id.toUpperCase()}
          </span>
        </button>
      ))}
    </div>
...
export default function TherapistFinancialView(): React.ReactElement {
  ...
  const { currentRange, currentSummary, records, isLoading, isLoadingRecords } =
    useFinancialData({
      period,
  ...
  const categoryChartData = useMemo(() => {
    ...
    const income = Object.entries(currentSummary.byCategory)
      ...
      .map(([key, v], i) => ({
        name:
          FINANCIAL_CATEGORIES[key as keyof typeof FINANCIAL_CATEGORIES]
            ?.label || key,
        value: v.income,
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    onClick={() =>
                      setFormData({ ...formData, accountType: "pj" })
                    }
                    type="button"
                  >
                  ...
                  <button
                    ...
                    onClick={() =>
                      setFormData({ ...formData, accountType: "cpf" })
                    }
                    type="button"
                  >
                ...
                >
                  {Object.entries(FINANCIAL_CATEGORIES)
                    .filter(([_, info]) => {
                      ...
                      if (info.type !== "both" && info.type !== formData.type)
                        return false;
          ...
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-900">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                onClick={() =>
                  deleteRecordMutation.mutate({ id: showDeleteConfirm })
                }
                type="button"
              >
