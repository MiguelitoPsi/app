...
export const ProfileView: React.FC<ProfileViewProps> = ({ onNavigate }) => {
  ...
  const getBadgeStatus = (badge: BadgeDefinition) => {
    ...
    const metricValue = typeof rawValue === "boolean" ? (rawValue ? 1 : 0) : typeof rawValue === "number" ? rawValue : 0;
    const percentage = badge.requirement > 0 ? Math.min(100, Math.max(0, (metricValue / badge.requirement) * 100)) : 100;
    ...
    <div className="relative h-full overflow-y-auto px-6 py-8 pb-24">
      ...
      <div className="mb-8 flex flex-col items-center">
        ...
        <div className="mb-4 space-y-1 text-center">
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="font-bold text-base text-violet-600 dark:text-violet-400">{currentRank.name}</span>
          </div>
        ...
        <div className="relative mt-2 max-w-xs rounded-2xl border border-violet-100 bg-violet-50 px-6 py-4 text-center dark:border-violet-900/30 dark:bg-violet-900/20">
          <Quote className="-top-2 -left-1 absolute rounded-full bg-white fill-current p-0.5 text-violet-300 dark:bg-slate-900 dark:text-violet-600" size={16} />
          <p className="font-medium text-violet-800 text-xs italic leading-relaxed dark:text-violet-200">"{dailyQuote}"</p>
        </div>
      </div>
      ...
      <div className="mx-1 mb-8 flex rounded-xl bg-slate-100 p-1 transition-colors dark:bg-slate-800">
        <button
          ...
          className={`flex flex-1 items-center justify-center gap-2 rounded-lg py-2.5 font-bold text-xs transition-all duration-200 sm:text-sm ${
            activeTab === "stats" ? "bg-white text-violet-600 shadow-sm dark:bg-slate-700" : "text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300"
          }`}
          onClick={() => setActiveTab("stats")}
        >
          <LayoutGrid size={16} /> Resumo
        </button>
        <button
          type="button"
          className={`flex flex-1 items-center justify-center gap-2 rounded-lg py-2.5 font-bold text-xs transition-all duration-200 sm:text-sm ${
            activeTab === "rank" ? "bg-white text-violet-600 shadow-sm dark:bg-slate-700" : "text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300"
          }`}
        ...
        <button
          type="button"
          className={`flex flex-1 items-center justify-center gap-2 rounded-lg py-2.5 font-bold text-xs transition-all duration-200 sm:text-sm ${
            activeTab === "achievements" ? "bg-white text-violet-600 shadow-sm dark:bg-slate-700" : "text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300"
          }`}
          ...
          <div className="slide-in-from-left-4 fade-in animate-in space-y-6 duration-300">
            <div className="grid grid-cols-3 gap-4">
              <div className="zoom-in flex animate-in flex-col items-center rounded-2xl border border-slate-100 bg-white fill-mode-backwards p-4 shadow-sm transition-colors delay-100 duration-500 dark:border-slate-700 dark:bg-slate-800">
                ...
                <span className="font-bold text-lg text-slate-800 dark:text-white">{stats.streak}</span>
                <span className="text-[10px] text-slate-400 uppercase tracking-wider dark:text-slate-500">Dias</span>
              </div>
              <div className="zoom-in flex animate-in flex-col items-center rounded-2xl border border-slate-100 bg-white fill-mode-backwards p-4 shadow-sm transition-colors delay-200 duration-500 dark:border-slate-700 dark:bg-slate-800">
                ...
                <span className="text-[10px] text-slate-400 uppercase tracking-wider dark:text-slate-500">Total XP</span>
              </div>
              <div
              ...
              >
                ...
                <span className="font-bold text-lg text-slate-800 dark:text-white">{stats.level}</span>
                <span className="text-[10px] text-slate-400 uppercase tracking-wider dark:text-slate-500">Nível</span>
              </div>
            </div>
            <div className="rounded-2xl border border-slate-100 bg-slate-50 p-6 text-center transition-colors dark:border-slate-800 dark:bg-slate-800/50">
              ...
              <p className="text-slate-500 text-sm leading-relaxed dark:text-slate-400">
                Você já completou <span className="font-bold text-violet-600 dark:text-violet-400">{stats.totalMeditationMinutes} min</span> de meditação e concluiu <span className="font-bold text-violet-600 dark:text-violet-400">{stats.totalTasksCompleted} tarefas</span>.<br />
          ...
          <div className="fade-in zoom-in animate-in duration-300">
            <div className="relative mb-8 overflow-hidden rounded-3xl bg-gradient-to-br from-violet-600 to-fuchsia-700 p-6 text-white shadow-lg shadow-violet-200 dark:shadow-none">
              ...
              <div className="relative z-10 mb-4 flex items-center justify-between">
                <div>
                  <p className="mb-1 font-bold text-violet-100 text-xs uppercase tracking-wider">Nível Atual</p>
              ...
              <div className="relative z-10">
                ...
                <div className="h-3 w-full overflow-hidden rounded-full bg-black/20 backdrop-blur-sm">
                  <div className="h-full bg-white shadow-[0_0_10px_rgba(255,255,255,0.5)] transition-all duration-1000 ease-out" style={{ width: `${levelProgressPercent}%` }} />
                </div>
                <p className="mt-3 text-violet-200 text-xs italic">Faltam {xpForNextLevel - stats.xp} XP para evoluir.</p>
              </div>
            </div>
            <div className="relative ml-4 space-y-4 border-slate-100 border-l-2 pl-4 transition-colors dark:border-slate-800">
              {RANKS.map((rank) => {
                ...
                return (
                  <div className={`relative py-1 pl-6 ${isUnlocked ? "opacity-100" : "opacity-60"}`} key={rank.level}>
                    <div
                    ...
                    >
                      <h4 className="font-bold text-sm">{rank.name}</h4>
                    ...
                    <p className="text-slate-500 text-xs leading-relaxed dark:text-slate-400">{rank.description}</p>
                    {isCurrent && (
        ...
        {activeTab === "achievements" && (
          <div className="slide-in-from-right-4 fade-in animate-in duration-300">
            <div className="mb-4 flex items-center justify-between px-1">
              <span className="flex items-center gap-2 font-bold text-slate-400 text-xs uppercase tracking-wider dark:text-slate-500">Coleção</span>
                  ...
                  <div key={catKey} className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      {categoryBadges.map((badge, index) => {
                        const { isUnlocked, currentProgress, progressPercentage } = getBadgeStatus(badge);
                        return (
                          <button
                            key={badge.id}
                          ...
                          >
                            ...
                            >
                              {isUnlocked && <div className="absolute inset-0 animate-pulse bg-violet-200 opacity-30" />}
                              <div className="relative z-10">{badge.icon}</div>
                            </div>
                            <div className="z-10 min-w-0 flex-1">
                              <div className={`truncate font-bold text-sm ${isUnlocked ? "text-slate-800 dark:text-white" : "text-slate-500 dark:text-slate-400"}`}>{badge.name}</div>
                              <div className="truncate text-slate-500 text-xs dark:text-slate-500">{badge.description}</div>
                              {!isUnlocked && badge.metric !== "auto" && (
                                <div className="mt-2 space-y-1">
                                  ...
                                  <div className="h-2 w-full overflow-hidden rounded-full border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
                                    <div className="h-full rounded-full bg-violet-400 transition-all duration-1000 ease-out dark:bg-violet-600" style={{ width: `${progressPercentage}%` }} />
                                  </div>
                                </div>
                              )}
                              {!isUnlocked && (
      ...
      {showSettings && (
        <div className="fade-in fixed inset-0 z-[100] flex animate-in items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm duration-200">
          <div className="zoom-in-95 relative w-full max-sm max-w-sm animate-in rounded-3xl border border-slate-100 bg-white p-6 shadow-2xl duration-300 dark:border-slate-800 dark:bg-slate-900" onClick={(e) => e.stopPropagation()}>
            <div className="mb-6 flex items-center justify-between">
              <h3 className="flex items-center gap-2 font-bold text-lg text-slate-800 dark:text-white">
                <Settings className="text-slate-400" size={20} /> Configurações
              </h3>
              <button type="button" className="rounded-full bg-slate-50 p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600 dark:bg-slate-800 dark:hover:text-slate-200" onClick={() => setShowSettings(false)}>
                <X size={20} />
              </button>
            </div>
            <div className="space-y-4">
              <div className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 p-4 transition-colors dark:border-slate-700 dark:bg-slate-800">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-800 text-sm dark:text-white">Modo Escuro</h4>
                    <p className="text-slate-500 text-xs dark:text-slate-400">Ajustar aparência do app</p>
                  </div>
                </div>
                <button
                  ...
                  onClick={toggleTheme}
                >
                  <span className={`absolute top-1 left-1 h-4 w-4 rounded-full bg-white shadow-sm transition-transform duration-300 ${stats.theme === "dark" ? "translate-x-6" : "translate-x-0"}`} />
                </button>
              </div>
              {onNavigate && (
                <button className="group flex w-full items-center justify-between rounded-xl border border-indigo-100 bg-indigo-50 p-4 transition-colors hover:bg-indigo-100 dark:border-indigo-800 dark:bg-indigo-900/20 dark:hover:bg-indigo-900/30" onClick={() => onNavigate(Tab.THERAPIST)}>
                  <div className="flex items-center gap-3">
                    <div className="rounded-lg bg-indigo-200 p-2 text-indigo-700 dark:bg-indigo-800 dark:text-indigo-300">
                      <Stethoscope size={20} />
                    </div>
                    <div className="text-left">
                      <h4 className="font-bold text-indigo-900 text-sm dark:text-indigo-200">Portal do Especialista</h4>
                      <p className="text-indigo-600 text-xs dark:text-indigo-400">Acessar painel do terapeuta</p>
                    </div>
                  </div>
                </button>
              )}
      ...
      {selectedBadge && selectedBadgeStatus && (
        <div className="fade-in fixed inset-0 z-[100] flex animate-in items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm duration-200">
          <div className="zoom-in-95 relative w-full max-w-sm animate-in rounded-3xl border border-slate-100 bg-white p-6 shadow-2xl duration-300 dark:border-slate-800 dark:bg-slate-900" onClick={(e) => e.stopPropagation()}>
            <button type="button" className="absolute top-4 right-4 rounded-full bg-slate-50 p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600 dark:bg-slate-800 dark:hover:bg-slate-700 dark:hover:text-slate-200" onClick={() => setSelectedBadge(null)}>
              <X size={20} />
            </button>
            <div className="mt-2 flex flex-col items-center text-center">
              <div className={`relative mb-4 overflow-hidden rounded-full p-6 text-5xl shadow-inner ${selectedBadgeStatus.isUnlocked ? "bg-violet-50 text-violet-600 shadow-violet-100 ring-4 ring-violet-50 dark:bg-violet-900/30 dark:text-violet-400 dark:shadow-none dark:ring-violet-900" : "bg-slate-100 text-slate-400 grayscale dark:bg-slate-800 dark:text-slate-600"}`}>
                {selectedBadgeStatus.isUnlocked && <div className="absolute inset-0 animate-pulse bg-violet-200 opacity-30" />}
                <div className="relative z-10">{selectedBadge.icon}</div>
              </div>
              <h3 className="font-bold text-slate-900 text-xl dark:text-white">{selectedBadge.name}</h3>
              <p className="mt-2 text-slate-500 leading-relaxed dark:text-slate-400">{selectedBadge.description}</p>
                  ...
                  <div className="space-y-1">
                    ...
                    {selectedBadgeStatus.unlockedDate && (
                      <div className="text-slate-500 text-sm dark:text-slate-400">
                        Desbloqueado em <span className="font-medium text-slate-700 dark:text-slate-300">{formatUnlockDate(selectedBadgeStatus.unlockedDate)}</span>
                      </div>
                    )}
                  </div>
                ) : (
                    ...
                    {selectedBadge.metric !== "auto" && (
                      <>
                        <div className="h-3 w-full overflow-hidden rounded-full border border-slate-300/50 bg-slate-200 dark:border-slate-600 dark:bg-slate-700">
                          <div className="h-full bg-violet-400 transition-all duration-500 ease-out dark:bg-violet-600" style={{ width: `${selectedBadgeStatus.progressPercentage}%` }} />
                        </div>
                        <div className="flex justify-between px-1 font-bold text-slate-500 text-xs dark:text-slate-400">
                          <span>Progresso atual</span>
                          <span>{selectedBadgeStatus.currentProgress} / {selectedBadge.requirement}</span>
                        </div>
                      </>
                    )}
                    <p className="pt-1 text-slate-400 text-xs italic dark:text-slate-500">Continue usando o app para desbloquear esta conquista.</p>
                  </div>
                )}
              </div>
              <button className="mt-6 w-full rounded-xl border border-slate-200 bg-white py-3 font-bold text-slate-600 text-sm transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700" onClick={() => setSelectedBadge(null)}>
                Fechar
              </button>
            </div>
          </div>
