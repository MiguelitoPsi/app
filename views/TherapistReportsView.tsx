...
export default function TherapistReportsView(): React.ReactElement {
  ...
  const [activeSection, setActiveSection] =
    useState<ReportSection>("documents");
  ...
  const [cognitiveForm, setCognitiveForm] =
    useState<CognitiveFormData>(initialFormData);
  ...
  const saveTherapeuticPlanMutation =
    trpc.therapistReports.saveTherapeuticPlan.useMutation({
      onSuccess: () => {
        utils.therapistReports.getCognitiveConceptualization.invalidate();
  ...
  const handleSaveCognitive = () => {
    ...
    saveCognitiveMutation.mutate({
      ...
      situations: {
        situation1: cognitiveForm.situation1.situation
          ? cognitiveForm.situation1
          : undefined,
        situation2: cognitiveForm.situation2.situation
          ? cognitiveForm.situation2
          : undefined,
        situation3: cognitiveForm.situation3.situation
  ...
  const handleDeleteCognitive = () => {
    if (!selectedPatientId) return;
    if (
      confirm("Tem certeza que deseja excluir esta conceituação cognitiva?")
    ) {
  ...
  const handleApproveAndGeneratePlan = async () => {
    ...
    try {
      ...
      const input: CognitiveConceptualizationInput = {
        ...
        conditionalAssumptions:
          cognitiveData.conditionalAssumptions || undefined,
        compensatoryStrategies:
          cognitiveData.compensatoryStrategies || undefined,
      ...
      setGenerationError(
        error instanceof Error
          ? error.message
          : "Erro ao aprovar e gerar plano terapêutico."
      );
    }
  };
  ...
  const handleGenerateTherapeuticPlan = async () => {
    ...
    try {
      ...
      setGenerationError(
        error instanceof Error
          ? error.message
          : "Erro ao gerar plano terapêutico."
      );
    }
  };
  ...
  const uploadMutation =
    trpc.therapistReports.uploadSessionDocument.useMutation({
      onSuccess: () => {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      alert(
        "Tipo de arquivo não suportado. Use PDF ou imagens (JPEG, PNG, WebP, GIF)."
      );
      return;
    }
                ...
                >
                  {isUploading ? (
                    <div className="flex flex-col items-center">
                      <div className="h-8 w-8 animate-spin rounded-full border-4 border-purple-200 border-t-purple-600" />
                      <span className="mt-2 text-purple-600 text-sm">
                        Enviando...
                      </span>
                    </div>
                  ) : (
      ...
      {showApprovalModal && (
        <div className="fade-in fixed inset-0 z-[100] flex animate-in items-center justify-center bg-slate-900/60 px-4 py-6 backdrop-blur-sm duration-200">
          ...
          >
            <div className="mb-4 flex items-center justify-between sm:mb-6">
              <h3 className="flex items-center gap-2 font-bold text-base text-slate-800 sm:text-lg dark:text-white">
                <Brain className="text-emerald-500" size={20} /> Confirmar
                Aprovação
              </h3>
              <button
                aria-label="Fechar modal"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-amber-700 text-xs dark:text-amber-400">
                      Ao confirmar, você declara que revisou as informações da
                      conceituação cognitiva e aprova o conteúdo gerado pela IA.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-violet-700 text-xs dark:text-violet-400">
                      A IA irá analisar a conceituação cognitiva aprovada e
                      gerar um plano terapêutico personalizado com sugestões de
                      intervenções e técnicas.
                    </p>
                  </div>
                </div>
