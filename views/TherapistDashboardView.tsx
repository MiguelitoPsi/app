...
import { RemixIconType } from "@/lib/utils/icon-map";
...
export const TherapistDashboardView: React.FC = () => {
  ...
  const [sessionForm, setSessionForm] =
    useState<SessionFormData>(defaultSessionForm);
  const [patientSearchQuery, setPatientSearchQuery] = useState("");
  const [showPatientDropdown, setShowPatientDropdown] = useState(false);

  // Consolidated query for all dashboard data (combines summary + pending items)
  const { data: dashboardData, isLoading } =
    trpc.analytics.getTherapistDashboardData.useQuery(undefined, {
      staleTime: 60_000,
    });
  ...
  const confirmPaymentMutation =
    trpc.therapistFinancial.confirmPayment.useMutation({
      onSuccess: () => {
        utils.analytics.getTherapistDashboardData.invalidate();
      },
    });
  ...
  const totalPendingItems =
    totalPendingRewards + totalUnpaidSessions + totalPendingJournals;
      ...
      <div className="grid min-h-0 flex-1 grid-cols-12 gap-5">
              ...
              {isLoading ? (
                ...
                <div className="flex flex-col gap-2 max-h-[200px] overflow-y-auto">
                  {dashboardData?.upcomingSessions?.map((session) => {
                    const sessionDate = session.dueDate
                      ? new Date(session.dueDate)
                      : null;
        ...
        <div className="col-span-12 flex flex-col gap-5 overflow-hidden lg:col-span-8">
          ...
          <Card>
            <div className="flex flex-row items-center justify-between pb-4 mb-4 border-b border-slate-100 dark:border-slate-700">
              <div className="flex flex-row items-center gap-2">
                ...
                {totalPendingItems > 0 && (
                  <CounterBadge count={totalPendingItems} />
                )}
              </div>
            </div>
                      ...
                      {dashboardData?.pendingJournals
                        ...
                        .map((journal) => (
                          <PendingItem
                            ...
                            key={journal.id}
                            text={`Registro de ${journal.patientName}${
                              journal.mood
                                ? ` (${translateMood(journal.mood)})`
                                : ""
                            }`}
                          />
                        ))}
          ...
          <Card className="flex-1 min-h-[200px]">
            <div className="flex flex-row items-center justify-between pb-4 mb-4 border-b border-slate-100 dark:border-slate-700">
              <div className="flex flex-row items-center gap-2">
                <RiCheckboxMultipleLine className="h-4 w-4 flex-shrink-0 text-slate-400" />
            ...
            <div className="relative bg-gradient-to-r from-sky-500 to-cyan-500 p-6 text-white">
              ...
              <p className="text-sky-100 text-sm">
                {selectedSessionPatient
                  ? `Sessão com ${selectedSessionPatient.name}`
                  : "Sessão"}
              </p>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    <div className="relative">
                      <RiSearch2Line className="absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 text-slate-400" />
                      <input
                ...
                <div>
                  ...
                  >
                    <Banknote className="mb-0.5 inline h-3 w-3" /> Valor da
                    Sessão (R$)
                  </label>
                  <input
                    ...
                    <div className="mt-2">
                      ...
                      <div className="grid grid-cols-7 gap-1">
                        {["D", "S", "T", "Q", "Q", "S", "S"].map(
                          (day, index) => {
                            const isSelected =
                              sessionForm.weekDays?.includes(index);
                            return (
                              <button
