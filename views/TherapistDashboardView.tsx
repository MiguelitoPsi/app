...
export const TherapistDashboardView: React.FC = () => {
  ...
  const { data: dashboardData, isLoading } = trpc.analytics.getTherapistDashboardData.useQuery(
    undefined,
    {
      staleTime: 60_000,
    }
    ...
    <div className='flex h-full w-full flex-col box-border overflow-hidden bg-slate-50 p-5 dark:bg-slate-900'>
      ...
      <div className='mb-6 flex items-center justify-between'>
        <div>
          ...
          <p className='text-slate-500 dark:text-slate-400'>Visão geral do seu consultório</p>
        </div>
        <button
                      ...
                      {dashboardData?.pendingJournals.slice(0, 5).map((journal) => (
                        <PendingItem
                          ...
                          text={`Registro de ${journal.patientName}${
                            journal.mood ? ` (${translateMood(journal.mood)})` : ''
                          }`}
                        />
                      ))}
                      ...
                      {sessionForm.frequency === 'weekly'
                        ...
                        : sessionForm.frequency === 'biweekly'
                          ...
                          : sessionForm.frequency === 'once' && sessionForm.dueDate
                            ? `Sessão em ${new Date(sessionForm.dueDate).toLocaleDateString(
                                'pt-BR',
                                { timeZone: 'UTC' }
                              )}`
                            : 'Tarefa adicionada'}
