...
import { TherapistDueNowSection } from "@/components/therapist";
...
export const TherapistDashboardView: React.FC = () => {
  ...
  const { data: statsData, isLoading: statsLoading } =
    trpc.therapistXp.getStats.useQuery(
      undefined,
      { staleTime: 2 * 60 * 1000 } // 2 minutes
    );
  const { data: activitySummary } =
    trpc.therapistXp.getActivitySummary.useQuery(
      undefined,
  ...
  const { data: achievementProgress } =
    trpc.therapistAchievements.getProgress.useQuery(undefined, {
  ...
  const unlockedAchievements =
    achievementProgress?.filter((a) => a.isUnlocked).length || 0;
                ...
                <div className="flex gap-1 rounded-xl bg-white p-1 shadow-sm dark:bg-slate-800">
                  {[
                    ...
                    <button
                      ...
                      key={tab.id}
                      onClick={() =>
                        setPatientSubTab(tab.id as typeof patientSubTab)
                      }
                      type="button"
                    >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50">
                      <p className="text-slate-500 text-sm dark:text-slate-400">
                        <strong>Atenção:</strong> Ao desvincular ou dar alta, a
                        conta do paciente será suspensa até que ele se vincule a
                        um novo terapeuta ou o admin exclua a conta.
                      </p>
                    </div>
                  </div>
                )}
                  ...
                  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="mx-4 w-full max-w-md rounded-xl bg-white p-6 shadow-2xl dark:bg-slate-800">
                      ...
                      <p className="mb-4 text-center text-slate-500 dark:text-slate-400">
                        ...
                        <strong>{selectedPatient?.name}</strong>? A conta do
