...
import { type Mood, Tab } from '../types'
...
export const HomeView: React.FC<HomeViewProps> = ({ changeTab }) => {
  const { stats, currentMood, setMood, tasks } = useGame()
  const [isXPAvailable, setIsXPAvailable] = useState(false)
  const [xpFeedback, setXpFeedback] = useState<{ id: number; amount: number } | null>(null)
  ...
  const currentRank = RANKS.find((r) => r.level === stats.level) || RANKS.at(-1)
      ...
      <div className='flex-1 space-y-6 overflow-y-auto px-6 py-6 pb-24'>
            ...
            <div className='flex items-center gap-3'>
              ...
              <div className='text-left'>
                ...
                <p className='font-medium text-red-600/80 text-xs dark:text-red-400/80'>
                  Você tem {urgentTasks.length} tarefa{urgentTasks.length > 1 ? 's' : ''} de alta
                  prioridade hoje.
                </p>
              </div>
            </div>
            <div className='flex h-8 w-8 items-center justify-center rounded-full bg-red-100 dark:bg-red-900/30'>
              <ArrowRight className='text-red-500 dark:text-red-400' size={16} />
            </div>
          </button>
        )}
        ...
        <div className='rounded-3xl border border-slate-100 bg-white p-5 shadow-sm transition-colors dark:border-slate-800 dark:bg-slate-900'>
          ...
          <div className='flex justify-between'>
            {moods.map((m) => (
              ...
              >
                <span className='text-3xl drop-shadow-sm filter'>{m.emoji}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className='grid grid-cols-2 gap-4'>
          ...
          <button
            ...
            onClick={() => changeTab(Tab.MEDITATION)}
          >
        ...
        <div className='rounded-3xl border border-slate-100 bg-white p-6 shadow-sm transition-colors dark:border-slate-800 dark:bg-slate-900'>
          <div className='mb-6 flex items-center gap-3'>
            ...
            <h3 className='font-bold text-slate-800 dark:text-white'>Humor Semanal</h3>
          </div>
          {isMounted && (
            <div className='h-40 w-full'>
              <ResponsiveContainer height='100%' width='100%'>
                <BarChart data={moodData}>
                  ...
                  />
                  <Bar barSize={24} dataKey='score' fill='#8b5cf6' radius={[6, 6, 6, 6]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
