...
export const MeditationView: React.FC = () => {
            ...
            return (
              ...
              >
                <div className='relative z-10 flex items-center gap-4 p-5'>
                  ...
                  <div
                    className={`flex h-8 w-8 items-center justify-center rounded-full bg-slate-50 text-slate-300 dark:bg-slate-800 dark:text-slate-600 group-hover:bg-${type.colorFrom.split('-')[1]}-50 dark:group-hover:bg-slate-700 group-hover:text-${type.colorFrom.split('-')[1]}-500 transition-colors`}
                  >
                    <Sparkles size={16} />
                  </div>
                </div>
  ...
  return (
    <div className='relative flex h-full flex-col overflow-hidden bg-slate-50 dark:bg-slate-950'>
      {/* Background Ambiance */}
      <div
        className={`absolute inset-0 z-0 transition-colors duration-[4000ms] ease-in-out ${phase === 'Inspirar' && isActive ? `${selectedType.bgActive}/80` : 'bg-slate-50 dark:bg-slate-950'}
      ...
      <div className='relative z-20 flex items-center justify-between px-6 pt-8 pb-6'>
        <button
          className={`flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white transition-all hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:hover:bg-slate-800 ${isActive ? 'pointer-events-none opacity-0' : 'opacity-100'}`}
          disabled={isActive}
          onClick={handleBack}
        >
        ...
        <div
          className={`flex flex-col items-center ${isActive ? 'opacity-0' : 'opacity-100'} transition-opacity duration-500`}
        >
          <div className={`flex items-center gap-2 ${selectedType.textColor} mb-1 opacity-80`}>
            <selectedType.icon size={16} />
            <span className='font-bold text-xs uppercase tracking-wider'>{selectedType.title}</span>
          </div>
          <h2 className='font-black text-slate-800 text-xl dark:text-white'>Respire Fundo</h2>
        </div>
      ...
      <div className='relative z-10 flex flex-1 flex-col items-center justify-center pb-20'>
        {/* Text Guide */}
        <div
          className={`absolute top-0 z-50 font-black text-3xl tracking-tight transition-all duration-500 ${isActive ? `${selectedType.textColor} translate-y-0 opacity-100` : 'translate-y-4 text-slate-300 opacity-0 dark:text-slate-600'}
        `}
        >
          {phase}
        </div>
        ...
        <div className='relative flex h-72 w-72 items-center justify-center'>
          ...
          {selectedType.id === 'relax' && (
            <>
              <div
                className={`absolute inset-0 rounded-full border opacity-30 transition-all delay-300 ${isActive ? getDurationClass() : 'duration-1000'}
                  ${getCircleScale()}border-${selectedType.colorFrom.split('-')[1]}-200 dark:border-${selectedType.colorFrom.split('-')[1]}-800 bg-${selectedType.colorFrom.split('-')[1]}-50 dark:bg-${selectedType.colorFrom.split('-')[1]}-900/20`}
              />
              <div
                className={`absolute inset-4 rounded-full border opacity-40 transition-all delay-150 ${isActive ? getDurationClass() : 'duration-1000'}
                  ${getCircleScale()}border-${selectedType.colorFrom.split('-')[1]}-300 dark:border-${selectedType.colorFrom.split('-')[1]}-700 bg-${selectedType.colorFrom.split('-')[1]}-100 dark:bg-${selectedType.colorFrom.split('-')[1]}-800/20`}
              />
            </>
          )}
          ...
          {selectedType.id === 'focus' && (
            <>
              <div
                className={`absolute inset-8 rounded-3xl border-2 opacity-30 transition-all delay-300 ${isActive ? getDurationClass() : 'duration-1000'}
                  ${getCircleScale()}border-${selectedType.colorFrom.split('-')[1]}-400 rotate-12`}
              />
              <div
                className={`absolute inset-8 rounded-3xl border-2 opacity-40 transition-all delay-150 ${isActive ? getDurationClass() : 'duration-1000'}
          ...
          {selectedType.id === 'anxiety' && (
            <>
              <div
                className={`absolute inset-12 rounded-xl border-4 opacity-30 transition-all delay-300 ${isActive ? getDurationClass() : 'duration-1000'}
                  ${getCircleScale()}border-${selectedType.colorFrom.split('-')[1]}-300 rotate-45`}
              />
              <div
                className={`absolute inset-12 rounded-xl border-4 opacity-40 transition-all delay-150 ${isActive ? getDurationClass() : 'duration-1000'}
          ...
          {selectedType.id === 'sleep' && (
            <>
              <div
                className={`absolute inset-0 rounded-full opacity-20 blur-xl transition-all delay-300 ${isActive ? getDurationClass() : 'duration-1000'}
                  ${getCircleScale()}bg-${selectedType.colorFrom.split('-')[1]}-400`}
              />
              <div
                className={`absolute inset-4 rounded-full opacity-30 blur-lg transition-all delay-150 ${isActive ? getDurationClass() : 'duration-1000'}
          ...
          {/* Main Core Shape */}
          <div
            className={`z-10 flex h-32 w-32 items-center justify-center shadow-2xl transition-all ${isActive ? getDurationClass() : 'duration-1000'}
            ${getCircleScale()}
            ${getCircleColor()}
            ...
            ${phase === 'Segurar' && isActive ? `ring-4 ${selectedType.ringColor} animate-pulse ring-opacity-50` : ''}
            `}
          >
            <span
              className={`font-light text-2xl text-white tracking-widest transition-opacity duration-300 ${isActive ? 'opacity-100' : 'opacity-0'}
            ...
            {!isActive && (
              <span
                className={`absolute font-bold text-slate-400 text-sm uppercase tracking-wider dark:text-slate-500 ${selectedType.id === 'anxiety' ? '-rotate-45' : ''}`}
              >
                Iniciar
              </span>
            )}
