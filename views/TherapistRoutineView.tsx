...
export default function TherapistRoutineView() {
  ...
  const { data: myTasks } = trpc.therapistTasks.getAll.useQuery(undefined, {
  ...
  const createTaskMutation = trpc.task.createForPatient.useMutation({
    onMutate: async (newOne) => {
      await utils.task.getPatientTasksFromTherapist.cancel({
        patientId: selectedPatientId || "",
      });
    ...
    onError: (_error, _newOne, context) => {
      ...
      if (_error.message.includes("Limite")) {
  ...
  const deleteTaskMutation = trpc.task.deletePatientTask.useMutation({
    onMutate: async (vars) => {
      ...
      if (selectedPatientId) {
        utils.task.getPatientTasksFromTherapist.setData(
          { patientId: selectedPatientId },
          (old) => (old ? old.filter((t) => t.id !== vars.taskId) : [])
        );
      }
      return { previousTasks };
    },
  ...
  const calendarTasks = (
    mainView === "patients" ? patientTasks || [] : myTasks || []
  ).filter((task) => task.dueDate !== null) as Array<{
    dueDate: Date | string;
    id?: string;
    type?: string;
    priority?: string;
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-slate-50 p-4 pt-safe sm:p-6 lg:p-5 dark:bg-slate-900">
          ...
          <div className="flex flex-col h-full lg:grid lg:grid-cols-12 lg:gap-5 overflow-hidden">
            ...
            <div className="mt-4 flex-1 min-h-0 lg:col-span-8 lg:mt-0 lg:h-full">
              <AgendaSidebar
                ...
                selectedDate={selectedDate}
                // @ts-expect-error Tasks já filtradas para remover null, type assertion necessária
                tasks={displayTasks.filter((t) => t.dueDate !== null)}
              />
            </div>
          </div>
        </div>
      ...
      {showTaskForm &&
        (mainView === "patients" || mainView === "my-routine") && (
