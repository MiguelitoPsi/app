    ...
    <div className='h-full overflow-y-auto bg-slate-50 px-4 pt-safe py-6 pb-28 sm:px-6 sm:py-8 sm:pb-32 dark:bg-slate-950'>
          ...
          {(mainView === 'my-routine' || selectedPatientId) && (
            <button
              ...
              onClick={() => {
                ...
                setTaskForm({ ...defaultTaskForm, dueDate: `${yyyy}-${mm}-${dd}` })
              }}
              type='button'
            >
              {showTaskForm ? (
      ...
      {mainView === 'patients' && (
        ...
        <div className='mb-4 sm:mb-6'>
          <button
            className='flex w-full items-center justify-between rounded-xl border border-slate-100 bg-white p-4 shadow-sm transition-colors hover:bg-slate-50 sm:rounded-2xl dark:border-slate-800 dark:bg-slate-900 dark:hover:bg-slate-800'
          ...
          {/* Patient Dropdown */}
          {showPatientList && (
            <div className='mt-2 max-h-60 overflow-y-auto rounded-xl border border-slate-100 bg-white shadow-xl dark:border-slate-800 dark:bg-slate-900'>
        ...
        <>
            ...
            <div className='fade-in slide-in-from-top-4 relative mb-6 animate-in overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 p-4 text-white shadow-lg shadow-emerald-200 sm:mb-8 sm:rounded-3xl sm:p-5 dark:shadow-none'>
              ...
              <div className='relative z-10 mb-2 flex items-end justify-between'>
                <div>
                  ...
                  <h3 className='font-bold text-xl sm:text-2xl'>{myDayProgress}% Concluído</h3>
                </div>
          ...
          <div className='space-y-2 sm:space-y-3'>
            ...
            {displayMyTasks.map((task, index) => {
              ...
              return (
                <div
                  className={`group slide-in-from-bottom-2 relative flex animate-in flex-col rounded-xl border-t border-r border-b border-l-4 bg-white fill-mode-backwards p-3 transition-all duration-300 sm:rounded-2xl sm:border-l-[6px] sm:p-4 dark:bg-slate-800 ${
                ...
                >
                  <div className='flex items-start justify-between'>
                    <div className='flex flex-1 items-center gap-3 sm:gap-4'>
                      <button
                        className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border-2 transition-all sm:h-8 sm:w-8 ${
                          task.status === 'completed'
                      ...
                      <div className='flex flex-col'>
                        <div className='flex flex-wrap items-center gap-2'>
                          <span
                            className={`font-bold text-sm sm:text-base ${task.status === 'completed' ? 'text-slate-400 line-through dark:text-slate-600' : 'text-slate-800 dark:text-slate-200'}`}
                          >
                            {task.title}
                          </span>
                          {displayDate && (
      ...
      {mainView === 'patients' && selectedPatientId ? (
        ...
        <div className='mt-8 flex flex-col items-center justify-center rounded-2xl border-2 border-slate-200 border-dashed bg-slate-50/50 p-8 text-center sm:mt-12 sm:rounded-3xl sm:p-12 dark:border-slate-800 dark:bg-slate-900/50'>
          <div className='mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-violet-100 text-violet-500 dark:bg-violet-900/30'>
            <Target size={32} />
          </div>
          <h3 className='mb-2 font-bold text-lg text-slate-700 dark:text-slate-300'>
            Selecione um Paciente
          </h3>
          <p className='max-w-[250px] text-slate-400 text-sm dark:text-slate-500'>
